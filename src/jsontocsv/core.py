        ...
        with open(output_file, 'w', newline='') as csvfile:
            ...
            for course_code, sections in self.courses.items():
                for section in sections:
                    ...
                    output_row = [str(course_code), section['section'], section['term'], section['status'], section['courseName'], section['location'], section['faculty'], (section['available'] if 'available' in section else ''), (section['capacity'] if 'capacity' in section else ''), section['credits'], section['academicLevel'], '-'.join(meeting_ids), flags['isMorning'], flags['isAfternoon'], flags['isEvening'], flags['onFriday'], flags['onThursday'], 1 if section['faculty'] == 'G. Klotz' else 0]
